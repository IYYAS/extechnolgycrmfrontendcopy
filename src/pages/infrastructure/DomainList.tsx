...
const DomainList: React.FC = () => {
    ...
    const fetchDomains = async (page: number = 1, search: string = '', currentFilters = filters) => {
        setLoading(true);
        setError(null);
        try {
            const data = await getProjectDomains(page, search, currentFilters);
    ...
    useEffect(() => {
        fetchDomains(currentPage, searchTerm, filters);
    }, [currentPage, filters]);
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in slide-in-from-top-4 duration-500 delay-150">
                <div className="bg-card border border-border p-6 rounded-[2rem] shadow-sm flex items-center gap-5 hover:border-primary/30 transition-colors">
                    ...
                    <div>
                        <p className="text-[10px] font-black text-muted uppercase tracking-widest mb-0.5">Total Domains</p>
                    ...
                    viewMode === 'table' ? (
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-border">
                                    {domains.map((domain, index) => {
                                        const daysLeft = domain.expiration_date ? Math.ceil((new Date(domain.expiration_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : null;
                                        ...
                                        return (
                                            <tr key={domain.id} className={`group transition-colors ${rowClass}`}>
                                                    ...
                                                    <div className="flex items-center space-x-3">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                <p className="text-foreground group-hover:text-primary transition-colors font-semibold text-sm uppercase tracking-tight">{domain.project_name || 'N/A'}</p>
                                                            ...
                                                            <p className="text-muted text-xs opacity-60">{domain.name || 'N/A'}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase border whitespace-nowrap ${getInvoiceStatusStyles(domain.invoice_status)}`}>
                                                        {domain.invoice_status?.replace('_', ' ') || 'NOT INVOICED'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <button
                                                        ...
                                                        className={`p-1 px-3 rounded-lg transition-all border inline-flex items-center gap-1.5 ${domain.invoice_status === 'INVOICED'
                                                                ? 'bg-blue-500/10 text-blue-500 border-blue-500/20'
                                                                : 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 border-emerald-500/20'
                                                            }`}
                                                        title={domain.invoice_status === 'INVOICED' ? 'Already Invoiced' : 'Generate Invoice'}
                                                    >
                                                        <Receipt size={14} />
                                                        <span className="text-[9px] font-black uppercase">
                                                            {domain.invoice_status === 'INVOICED' ? 'Billed' : 'Bill'}
                                                        </span>
                                                    </button>
                                                </td>
                                                <td className="px-6 py-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                            {domains.map((domain) => {
                                ...
                                return (
                                    <div key={domain.id} onClick={() => navigate(`/infrastructure/domains/${domain.id}`)} className={`group rounded-2xl p-5 transition-all hover:shadow-xl hover:shadow-primary/5 relative overflow-hidden cursor-pointer border ${cardClass}`}>
                                        {(daysLeft !== null && daysLeft <= 30) && (
                                        ...
                                        <div className={`absolute top-0 right-0 p-4 flex items-center gap-2 ${daysLeft !== null && daysLeft <= 30 ? 'mt-2' : ''}`}>
                                            ...
                                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase border ${getInvoiceStatusStyles(domain.invoice_status)}`}>
                                                {domain.invoice_status === 'INVOICED' ? 'Invoiced' : 'Pending Bill'}
                                            </span>
                                        </div>
                                        <div className={`flex items-start space-x-4 mb-4 ${daysLeft !== null && daysLeft <= 30 ? 'mt-3' : ''}`}>
                                            ...
                                            <div className="pr-12">
                                                <h3 className="text-foreground font-bold text-base group-hover:text-primary transition-colors line-clamp-1 uppercase tracking-tight">{domain.project_name || 'N/A'}</h3>
                                                <p className="text-muted text-xs font-medium">{domain.name || 'N/A'}</p>
                                            </div>
                                        </div>
