                ...
                >
                    <div className="flex-1 space-y-4" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] text-indigo-500 font-bold block uppercase tracking-wider">Service Allocation End</label>
                                <input type="date"
                                    disabled={!hasPermission('change_projectservice')}
...
const ProjectForm: React.FC = () => {
                    ...
                    const cleanedData = {
                        ...
                        services: projectData.services?.map((s: any) => ({
                            ...
                            members: s.members?.map((m: any) => ({
                                ...
                                employee_name: m.employee_name || m.employee_detail?.first_name || m.employee_detail?.username || usersData.results.find((u: any) => u.id === m.employee)?.first_name
                            }))
                        })),
                        project_teams: projectData.project_teams?.map((pt: any) => ({
                            ...pt,
                            members: pt.members?.map((m: any) => {
                                const userInList = usersData.results.find((u: any) => u.id === m.employee);
                                let name = m.employee_name || userInList?.first_name || userInList?.username;
                                if (!name && pt.team_detail && pt.team_detail.members && pt.team_detail.member_names) {
                                    const tIdx = pt.team_detail.members.indexOf(m.employee);
                                    if (tIdx !== -1) name = pt.team_detail.member_names[tIdx];
                                }
                                return { ...m, employee_name: name };
                            })
                        })),
    ...
    const handleAddNature = async () => {
        if (!hasPermission('add_projectnature')) {
            alert('You do not have permission to add a project nature.');
            return;
        }
    ...
    const handleDeleteNature = async () => {
        if (!hasPermission('delete_projectnature')) {
            alert('You do not have permission to delete a project nature.');
            return;
        }
        if (!formData.project_nature) {
            alert('Please select a nature to delete.');
            return;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            // Auto fill placeholder texts if descriptions are empty
            const finalFormData = { ...formData };
            if (!finalFormData.description || finalFormData.description.trim() === '') {
                finalFormData.description = "this project now what happening";
            }
            if (finalFormData.project_base_informations && finalFormData.project_base_informations.length > 0) {
                finalFormData.project_base_informations = finalFormData.project_base_informations.map((base: any) => ({
                    ...base,
                    description: (!base.description || base.description.trim() === '') ? "What is this component about?" : base.description
                }));
            }

            // Prevent submitting empty placeholder items for lists
            if (finalFormData.project_documents) {
                finalFormData.project_documents = finalFormData.project_documents.filter((doc: any) => doc.name && doc.name.trim() !== '');
            }
            if (finalFormData.project_clients) {
                finalFormData.project_clients = finalFormData.project_clients.filter((c: any) => c.company_name && c.company_name.trim() !== '');
            }
            if (finalFormData.project_business_addresses) {
                finalFormData.project_business_addresses = finalFormData.project_business_addresses.filter((a: any) =>
                    a.id || (a.legal_name && a.legal_name.trim() !== '') || (a.attention_name && a.attention_name.trim() !== '')
                );
            }
            ...
            if (finalFormData.project_excutions) {
                finalFormData.project_excutions = finalFormData.project_excutions.filter((e: any) =>
                    (e.work_assigned_date && e.work_assigned_date.trim() !== '') ||
                    (e.start_date && e.start_date.trim() !== '') ||
                    (e.assigned_delivery_date && e.assigned_delivery_date.trim() !== '') ||
                    (e.confirmed_end_date && e.confirmed_end_date.trim() !== '')
                );
            }

            const bizAddrId = finalFormData.project_business_addresses?.[0]?.id;
            const projectId = isEdit && id ? parseInt(id) : undefined;
            const projectName = finalFormData.project_base_informations?.[0]?.name || finalFormData.name;
            const payload = {
                ...finalFormData,
                name: projectName,
                project_domains: finalFormData.project_domains.map((d: any) => ({ ...d, client_address: bizAddrId, project: projectId })),
                project_servers: finalFormData.project_servers.map((s: any) => ({ ...s, client_address: bizAddrId, project: projectId })),
                project_teams: finalFormData.project_teams?.map((pt: any) => ({
                    ...pt,
                    project: projectId,
                    members: pt.members?.map((m: any) => ({ ...m, project: projectId }))
                })),
                services: finalFormData.services?.map((sv: any) => ({
                    ...sv,
                    project: projectId,
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-500 mb-24">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {hasPermission('view_projectbaseinformation') && (
                    <FormSection title="Project Foundation" icon={<Briefcase size={22} />} iconColor="text-primary" bgColor="bg-primary/10" defaultOpen
                        action={
                            (hasPermission('add_projectbaseinformation') && (!formData.project_base_informations || formData.project_base_informations.length === 0)) ? (
                        ...
                        <div className="space-y-8">
                            {formData.project_base_informations?.map((base: any, idx: number) => (
                                <div key={idx} className={`space-y-6 ${idx > 0 ? 'pt-8 border-t border-border' : ''}`}>
                                    <div className="flex justify-between items-center">
                                        ...
                                        {idx > 0 && hasPermission('delete_projectbaseinformation') && (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="md:col-span-2 space-y-2">
                                            ...
                                            <input
                                                ...
                                                value={base.name || ''}
                                                disabled={!hasPermission('change_projectbaseinformation')}
                                        ...
                                        <div className="md:col-span-2 space-y-2">
                                            ...
                                            <textarea
                                                rows={3} placeholder="What is this component about?"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-border/50">
                                ...
                                {hasPermission('view_projectnature') && (
                                            ...
                                            {isAddingNature ? (
                                                <div className="flex-1 flex gap-2 animate-in fade-in slide-in-from-right-2 duration-300">
                                                    <input
                                                        ...
                                                        value={newNatureName}
                                                        disabled={!hasPermission('add_projectnature')}
                                                ...
                                                <>
                                                    ...
                                                    {hasPermission('add_projectnature') && (
                                                        <button
                                                            ...
                                                            className="p-3 shrink-0 bg-primary/10 text-primary rounded-xl hover:bg-primary/20 transition-all border border-primary/20"
                                                            title="Add New Nature"
                                                        >
                                                            <Plus size={20} />
                                                        </button>
                                                    )}
                                                    {hasPermission('delete_projectnature') && formData.project_nature && (
                                                        <button
                                                            type="button"
                                                            onClick={handleDeleteNature}
                                                            className="p-3 shrink-0 bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500/20 transition-all border border-rose-500/20"
                                                            title="Delete Selected Nature"
                                                        >
                                                            <Trash2 size={20} />
                                                        </button>
                                                    )}
                                ...
                                <div className="md:col-span-2 space-y-2">
                                    ...
                                    <textarea
                                        rows={3} placeholder="this project now what happening"
                ...
                {hasPermission('view_projectfinance') && (
                    <FormSection title="Financial Overview" icon={<DollarSign size={22} />} iconColor="text-emerald-500" bgColor="bg-emerald-500/10" defaultOpen
                        action={
                            hasPermission('add_projectfinance') ? (
                            ...
                            {formData.project_finances?.map((fin: any, idx: number) => (
                                <div key={idx} className={`space-y-4 ${idx > 0 ? 'pt-8 border-t border-border' : ''}`}>
                                    <div className="flex justify-between items-center">
                                        ...
                                        {idx > 0 && hasPermission('delete_projectfinance') && (
