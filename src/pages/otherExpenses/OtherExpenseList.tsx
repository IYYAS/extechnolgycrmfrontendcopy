...
import { getOtherExpenses, deleteOtherExpense } from './otherExpenseService';
import type { OtherExpense } from './otherExpenseService';
import {
    Search,
    Plus,
    Calendar,
...
const OtherExpenseList: React.FC = () => {
    ...
    const fetchData = async (page: number = 1, search: string = '') => {
        ...
        setError(null);
        try {
            const data = await getOtherExpenses(page, search);
            setExpenses(data.results);
            setTotalCount(data.count);
        } catch (err: any) {
            console.error('Failed to fetch other expenses:', err);
            setError(err.response?.data?.detail || 'Failed to load other expenses.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const totalAmount = expenses.reduce((acc, exp) => acc + parseFloat(exp.amount), 0).toFixed(2);

    if (loading && expenses.length === 0) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
            <Loader2 className="animate-spin text-primary" size={48} />
            <p className="text-muted font-medium italic">Loading other expenses...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-foreground bg-gradient-to-r from-rose-500 to-rose-400 bg-clip-text text-transparent">Other Expenses</h1>
                    <p className="text-muted font-bold text-sm uppercase tracking-widest mt-1">Manage miscellaneous expenses</p>
                </div>
                <button onClick={() => navigate('/other-expenses/new')} className="flex items-center gap-2 px-6 py-3 bg-rose-500 text-white font-black rounded-2xl shadow-xl shadow-rose-500/30 hover:shadow-rose-500/50 transition-all hover:scale-[1.02] active:scale-95">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-card border border-border p-6 rounded-[2.5rem] shadow-sm relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-8 text-rose-500/5 group-hover:text-rose-500/10 transition-colors"><DollarSign size={80} /></div>
                    <p className="text-[10px] font-black uppercase text-muted tracking-[0.2em]">Total Amount Extracted</p>
                    <h2 className="text-4xl font-black mt-2 text-rose-500">${totalAmount}</h2>
            ...
            <div className="bg-card border border-border rounded-[3rem] shadow-sm overflow-hidden min-h-[400px]">
                                ...
                                <tr key={expense.id} className="hover:bg-muted/5 transition-colors group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-3">
                                            <div className="p-2.5 bg-rose-500/10 text-rose-500 rounded-xl"><Calendar size={18} /></div>
                                            <div>
                                                <p className="text-sm font-black text-foreground">{expense.date}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                    ...
                                    <td className="px-8 py-6">
                                        <p className="text-sm font-black text-rose-500">${expense.amount}</p>
                                    </td>
                                    <td className="px-8 py-6">
                                        <p className="text-sm text-foreground/80 line-clamp-2 max-w-xs font-medium">{expense.notes || '—'}</p>
                                    </td>
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                                            <button onClick={() => navigate(`/other-expenses/edit/${expense.id}`)} className="p-2 hover:bg-rose-500/10 text-rose-500 rounded-xl transition-colors border border-transparent hover:border-rose-500/20"><Edit2 size={16} /></button>
                                            <button onClick={() => handleDelete(expense.id)} className="p-2 hover:bg-rose-500/10 text-rose-500 rounded-xl transition-colors border border-transparent hover:border-rose-500/20"><Trash2 size={16} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                {expenses.length === 0 && (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-muted text-sm max-w-xs mx-auto">Try adjusting your search term or add a new other expense record.</p>
                    </div>
                )}
                    ...
                    <div className="px-8 py-6 bg-muted/5 border-t border-border flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="p-2 hover:bg-rose-500/10 text-rose-500 rounded-xl disabled:opacity-30 transition-all border border-border hover:border-rose-500/20"
                            >
                                <ChevronLeft size={20} />
                            </button>
                            <span className="px-4 py-2 bg-rose-500/5 text-rose-500 text-sm font-black rounded-xl border border-rose-500/20">{currentPage} / {totalPages}</span>
                            <button
